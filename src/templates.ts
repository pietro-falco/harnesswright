...
const CLAUDE_MD = `# Agent session guide

This harness was generated by harnesswright. Sessions working in this
repository follow the stack's agent contract:

- **ADR gate** — implement only against an Accepted ADR; two-commit
  lifecycle (Proposed, then Accepted, then implementation).
- **Evidence discipline** — evidence is raw output (exit codes, literal
  stdout, \`git show HEAD:<path>\`); a prose summary is never evidence.
- **Git hygiene** — explicit-path staging only, never \`--no-verify\`,
  no destructive operations without an explicit operator directive.
- **Scope and stops** — a failed gate is a full stop, never an
  auto-retry; a directive is a ceiling, not a launch point.

The authoritative Agent contract lives in harness-pack:
\`docs/STACK.md\` § Agent contract. Where this file and that section
differ, that section wins.

Gate for this repository: \`harnesswright gate [slice-id]\` — run it
